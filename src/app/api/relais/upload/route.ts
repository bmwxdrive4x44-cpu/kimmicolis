...
const UPLOAD_DIR = path.join(process.cwd(), 'public', 'uploads', 'commerce-docs');
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (session.user.role !== 'RELAIS') {
      return NextResponse.json(
        { error: 'Accès refusé: seuls les points relais peuvent uploader des documents' },
        { status: 403 }
      );
    }
    ...
    const relativePath = `/uploads/commerce-docs/${filename}`;
